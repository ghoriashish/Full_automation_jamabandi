    ...
    def Hitting_url(self, url, Main, is_paginated):
        ...
        try:
            ...
            for i in range(2,len(check_total_district)):
                dictrict_name = tree_district.xpath(f"//*[contains(text(),'Select District')]//..//select//option[{i}]//text()")[0]
                dictrict_code = tree_district.xpath(f"//*[contains(text(),'Select District')]//..//select//option[{i}]//@value")[0]
                select_district_clik = self.diver.find_element(By.XPATH, f"//*[contains(text(),'Select District')]//..//select//option[{i}]")
                ...
                for j in range(2, len(check_total_tehsil)):
                    ...
                    select_tehsil_clik = self.diver.find_element(By.XPATH,f"//*[contains(text(),'Select Tehsil/ Sub-Tehsil')]//..//select//option[{j}]")
                    ...
                    for k in range(2, len(check_total_village)):
                        ...
                        select_village_clik = self.diver.find_element(By.XPATH,f"//*[contains(text(),'Select Village')]//..//select//option[{k}]")
                        ...
                        for l in range(2, len(check_khasra_total)):
                            try:
                                ...
                                inner_details_list.append(f'district(िजला):{dictrict_name}')
                                inner_details_list.append(f'year (साल):{year}')
                                inner_details = ' | '.join(inner_details_list)
                                if khasra_number != '':
                                    "---------insert data----------"
                                    data_list.append(dictrict_name)
                                    data_list.append(dictrict_code)
...
if __name__ == "__main__":
    Class_obj = FullAutomationJamabandi()
    Class_obj.Initiate()
